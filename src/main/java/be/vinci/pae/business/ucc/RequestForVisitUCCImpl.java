...
public class RequestForVisitUCCImpl implements RequestForVisitUCC {
   ...
   */
  @Override
  public List<RequestForVisitDTO> listRequestByUserId(int userId) {
    List<RequestForVisitDTO> dtos;
    try {
      dalServices.startTransaction();
      dtos = requestForVisitDAO.findByUserId(userId);
      for (RequestForVisitDTO dto : dtos) {
        completeFurnitureDTO(dto);
      }
      dalServices.commitTransaction();
   ...
   */
  @Override
  public RequestForVisitDTO cancelRequest(int idRequest) {
    ...
    try {
      ...
      if (request.getRequestStatus() != RequestStatus.WAITING) {
        throw new ConflictException("The request status can not be modify");
      }
   ...
   */
  @Override
  public RequestForVisitDTO acceptRequest(int idRequest) {
    RequestForVisitDTO request;
    try {
      dalServices.startTransaction();
      request = requestForVisitDAO.findByRequestId(idRequest);
